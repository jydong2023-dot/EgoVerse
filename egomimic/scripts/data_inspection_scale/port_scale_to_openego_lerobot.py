...
class EgoVerseEpisodeReader:
    ...
    def _shard_file_for_coords(self, key: str, coords: tuple[int, ...]) -> Path:
        chunk_root = self._key_dir(key) / "c"
        path = chunk_root.joinpath(*(str(coord) for coord in coords))
        if path.exists():
            return path
        if len(coords) == 1:
            dotted = chunk_root / str(coords[0])
        else:
            dotted = chunk_root / ".".join(str(coord) for coord in coords)
        if dotted.exists():
            return dotted
        raise FileNotFoundError(
            f"No shard file found for {key} at outer chunk {coords}"
        )

    def _decode_chunk_codecs(
        self,
        chunk_bytes: bytes,
        codec_specs: list[dict[str, Any]],
    ) -> bytes | np.ndarray:
    ...
    def _decode_chunk(self, chunk_bytes: bytes, meta: dict[str, Any]) -> np.ndarray:
        ...
        payload = self._decode_chunk_codecs(chunk_bytes, codec_specs)
    ...
    def _chunk_file_for_coords(self, key: str, coords: tuple[int, ...]) -> Path:
        chunk_root = self._key_dir(key) / "c"
        path = chunk_root.joinpath(*(str(coord) for coord in coords))
        if path.exists():
            return path
        ...
        if dotted.exists():
            return dotted
        raise FileNotFoundError(f"No chunk file found for {key} at chunk {coords}")

    def _decode_regular_chunk_array(self, key: str) -> np.ndarray:
        ...
        grid_shape = tuple(
            ceil_div(dim, chunk) for dim, chunk in zip(shape, chunk_shape, strict=False)
        )
        codec_specs = meta["codecs"]
        ...
        for chunk_coords in np.ndindex(*grid_shape):
            try:
                chunk_file = self._chunk_file_for_coords(key, chunk_coords)
            except FileNotFoundError:
                continue

            payload = self._decode_chunk_codecs(chunk_file.read_bytes(), codec_specs)
            if meta["data_type"] == "variable_length_bytes":
                decoded = np.asarray(payload, dtype=object)
            ...
            for coord, chunk, dim in zip(
                ...
                decoded_slices.append(slice(0, stop - start))
            else:
                out[tuple(out_slices)] = decoded[tuple(decoded_slices)]
        return out
    ...
    def _decode_sharded_array(self, key: str) -> np.ndarray:
        ...
        outer_chunk_shape = tuple(
            meta["chunk_grid"]["configuration"].get("chunk_shape", shape)
        )
        chunks_per_shard = tuple(
            ceil_div(outer, inner)
            for outer, inner in zip(outer_chunk_shape, inner_chunk_shape, strict=False)
        )
        ...
        outer_grid_shape = tuple(
            ceil_div(dim, outer)
            for dim, outer in zip(shape, outer_chunk_shape, strict=False)
        )
        for outer_coords in np.ndindex(*outer_grid_shape):
            try:
                shard_file = self._shard_file_for_coords(key, outer_coords)
            except FileNotFoundError:
                continue
            ...
            for inner_coords in np.ndindex(*chunks_per_shard):
                offset, length = index[inner_coords]
                if offset == MAX_UINT64 or length == MAX_UINT64 or length == 0:
                    continue
                ...
                out_slices = []
                decoded_slices = []
                for outer_coord, inner_coord, outer_size, inner_size, dim in zip(
                    outer_coords,
                    inner_coords,
                    outer_chunk_shape,
                    inner_chunk_shape,
                    ...
                    start = outer_coord * outer_size + inner_coord * inner_size
                    stop = min(start + inner_size, dim)
                    if start >= dim:
                        break
                    out_slices.append(slice(start, stop))
                    decoded_slices.append(slice(0, stop - start))
    ...
    def get_array(self, key: str) -> np.ndarray:
        ...
        if npy_path.exists():
            ...
            meta = self._load_key_meta(key)
            if meta["codecs"][0]["name"] == "sharding_indexed":
                arr = self._decode_sharded_array(key)
            else:
                arr = self._decode_regular_chunk_array(key)
        self._cache[key] = arr
        return arr
